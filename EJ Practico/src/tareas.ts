...
import { obtenerTareas, obtenerestados, Tarea } from "./data.js";
...
export function ingresarDatos(): {
    ...
    let titulo: string = prompt("Ingrese el título de la tarea: ").trim();
    while (titulo === "") {
        console.log("⚠️ El título no puede estar vacío.");
        titulo = prompt("Ingrese el título de la tarea: ").trim();
    }

    let descripcion: string = prompt("Ingrese la descripción de la tarea: ").trim();
...
export function mostrarDetallesYEditar(tarea: Tarea): void {
    ...
    if (op.toLowerCase() === "e") editarTarea(tarea);
...
export function mostrarTareasPorEstado(tipo: string): void {
    let lista: Tarea[];
    ...
    let nStr: string = prompt("Ingrese el número de la tarea para ver detalles o 0 para volver: ").trim();
